import rawPosts from "virtual:media-posts"
...
const mediaPosts: MediaPost[] = (
  rawPosts as { fileName: string; content: string }[]
)
  .map((raw) => {
...
export function getMediaBySlug(slug: string): MediaPost | undefined {
  return mediaPosts.find((m) => m.slug === slug)
}
